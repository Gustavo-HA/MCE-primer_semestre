...
qqnormalplot <- function(D, banda = NULL){
  ...
  if (!is.null(banda)){
    titulo <- TeX(paste0(r"(Gráfica Q-Q Normal : $1-\alpha =$)",banda))
  }else{
    titulo <- "Gráfica Q-Q Normal"
  }
  ...
  p <- ggplot(D) +
    ...
    # Parámetros de visualización
    labs(title = titulo, x = "Cuantil teórico", y = "Observaciones", color = "Datos") +
    theme(panel.background = element_blank(),
          panel.border = element_rect(colour="black",fill = NA, linewidth = 1),
          plot.title = element_text(hjust = 0.5), legend.position = c(0.27,0.84),
  ...
  if (!is.null(banda)){
    ...
    p <- p +
      geom_line(data = D, mapping = aes(x = z_i, y = bandasuperior, color = "Bandas de confianza"), linetype = "dashed") +
      geom_line(data = D, mapping = aes(x = z_i, y = bandainferior, color = "Bandas de confianza"), linetype = "dashed")
  }
  
  p <- p + 
    scale_color_manual(values = c("Muestra" = "black", "Bandas de confianza" = "blue"))
