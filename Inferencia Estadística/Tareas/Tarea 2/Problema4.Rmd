...
Vamos a hacer una función para reproducir las gráficas.
...
p4_graficas <- function(tabla_frecuencia,
                        ...
                        ancho=4, alto=4,
                        ticks_x = NULL,
                        dist_con_linea = FALSE){
  png(
    filename = paste0(ruta_imagenes, nombre_grafica),
    units = "in",
    width = ancho,
    height = alto,
    res = 300
  ...
  lines(y = dgeom(0:max(as.integer(names(tabla_frecuencia))), prob = p),
    x = c(1:(max(as.integer(names(tabla_frecuencia)))+1))+0.2,
    type = tipo,
...
# Graficamos la distirbución geométrica desde 0 hasta el valor máximo
# que haya resultado de nuestra simulación, en este caso fue 15.
lines(y = dgeom(0:max(as.integer(names(salida1))), prob = p),
    x = c(1:(max(as.integer(names(salida1)))+1))+0.2,
    type = "h",
    col = "#000000", lty = "dashed"
)
...
# Graficamos la distirbución geométrica desde 0 hasta el valor máximo
# que haya resultado de nuestra simulación.
lines(y = dgeom(0:max(as.integer(names(salida2))), prob = p),
    x = c(1:(max(as.integer(names(salida2)))+1))+0.2,
    type = "h",
    col = "#000000", lty = "dashed"
)

axis(side = 1, at = seq(from = 0, to = 50, by = 5))
